...
Асинхронный Telegram бот, автоматически находящий и отправляющий в чат новые медиафайлы с [Danbooru (Kagamihara)](https://kagamihara.donmai.us/) по указанным пользователем тегам и настройкам, индивидуальным для каждого чата.
...
| **База данных**                     | [Peewee ORM](https://docs.peewee-orm.com/) + **SQLite**          | Хранение тегов, настроек чатов, статусов постов и очередей               |
| **Планировщик задач**               | [APScheduler](https://apscheduler.readthedocs.io/)               | Автоматический запуск задач по расписанию                                |
| **HTTP-запросы**                    | [Aiohttp](https://docs.aiohttp.org/)                             | Асинхронные запросы к Danbooru API                                       |
| **Обработка изображений**           | [Pillow (PIL)](https://pillow.readthedocs.io/)                   | Изменение размера и обработка изображений перед отправкой                |
| **Логирование**                     | Встроенный `logging`                                             | Логи работы бота и отладочная информация                                 |
| **Архитектура проекта**             | Модульная структура (`handlers`, `utils`, `filters`, `database`) | Разделение логики                                                        |
| **Язык**                            | Python 3.10+                                                     | Язык разработки                                                          |
| **Инфраструктура**                  | Docker (опционально)                                             | Удобный деплой и изоляция окружения                                      |


## 📖 История создания и планы по развитию

Я веду Telegram канал и решила автоматизировать процесс поиска нового контента, чтобы вручную не просматривать каждый тег на Danbooru и ничего не пропустить. Для этого и был разработан Booru Feed Bot. Сейчас бот стабильно работает в нескольких чатах. Однако с ростом числа чатов начали появляться проблемы. В ближайших планах — реализация очередей и кэширования, чтобы обеспечить масштабируемость и стабильную работу при высокой нагрузке.
...
- **Добавить теги:** `/add_tag` (укажите один или несколько тегов через пробел). В начале можете указать дату, с которой будет осуществляться поиск медиафайлов. По умолчанию поиск начинается с момента запуска бота в чате.
    
  **Примеры:** `/add_tag 1girl landscape`; `/add_tag 2025-09-11 1girl`
...
### Как начать пользоваться ботом в Telegram
1. Добавьте бота в ваш чат.
2. Запустите поиск медиа через команду `/start_fetch`.
3. Настройте список тегов через команду `/add_tag` и укажите один или несколько тегов через пробел. В начале можете указать дату, с которой начнётся поиск в формате `2025-09-11`.

> ⚠️ **Примечание:** по умолчанию, без указания даты, поиск новых картинок по тегам начнется через заданный интервал от времени, когда вы ввели команду `/add_tag`. Популярный тег для проверки: `1girl`.
